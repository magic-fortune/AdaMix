...
from dataloaders.mmhws import (BaseDataSets, TwoStreamBatchSampler, WeakStrongAugment)
...
parser.add_argument('--root_path', type=str, default='../data/MMHWS/datasets', help='Name of Experiment')
...
parser.add_argument('--m', help='(max + min)/m', default=3)
...
def get_ACDC_2DLargestCC(segmentation):
    ...
    for i in range(N):
        ...
        for c in range(1, 8):  # Class labels start from 1
...
def patients_to_slices(dataset, patiens_num):
    ...
    elif "MMHWS" in dataset:
        ref_dict = {"1": 40, "5": 192, "10": 383}
        # ref_dict = {"1": 35, "5": 173, "10": 387}
    else:
        print("Error")
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            with torch.no_grad():
                ...
                plab_a = get_ACDC_masks(pre_a, nms=1)  # plab_a.shape=[6, 256, 256]
                plab_b = get_ACDC_masks(pre_b, nms=1)
                
                pre_a_s = ema_model(uimg_a_s)
                pre_b_s = ema_model(uimg_b_s)
                plab_a_s = get_ACDC_masks(pre_a_s, nms=1)  # plab_a.shape=[6, 256, 256]
            ...
            mask_model_1_a = (max_a < max_a_mean.unsqueeze(1).unsqueeze(2).expand(2, 256, 256)).int().unsqueeze(1)
            mask_model_1_b = (max_b < max_b_mean.unsqueeze(1).unsqueeze(2).expand(2, 256, 256)).int().unsqueeze(1)
            mask_model_2_a = (max_c < max_c_mean.unsqueeze(1).unsqueeze(2).expand(2, 256, 256)).int().unsqueeze(1)
            mask_model_2_b = (max_d < max_d_mean.unsqueeze(1).unsqueeze(2).expand(2, 256, 256)).int().unsqueeze(1)
...
if __name__ == "__main__":
    ...
    # -- path to save models
    pre_snapshot_path = "./model/AdaMix/mmhws_2_AdaMix_AdaMix_5_m_3_lam_1_labeled_exange_fea_out_final_have_ori_cps_loss_REE/pre_train"
    self_snapshot_path = "./model/AdaMix/mmhws_AdaMix_{}_{}_m_{}_lam_{}_labeled_exange_fea_out_final_have_ori_cps_loss_RE/self_train".format(args.exp, args.labelnum,  args.m, args.lam)
