...
def get_ACDC_2DLargestCC(segmentation):
    ...
    for i in range(N):
        ...
        for c in range(1, 4):  # Class labels start from 1
...
def self_train(args ,pre_snapshot_path, snapshot_path):
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            mask_model_1_a = (max_a < max_a_mean.unsqueeze(1).unsqueeze(2).expand(6, 256, 256)).int().unsqueeze(1)
            mask_model_1_b = (max_b < max_b_mean.unsqueeze(1).unsqueeze(2).expand(6, 256, 256)).int().unsqueeze(1)
            mask_model_2_a = (max_c < max_c_mean.unsqueeze(1).unsqueeze(2).expand(6, 256, 256)).int().unsqueeze(1)
            mask_model_2_b = (max_d < max_d_mean.unsqueeze(1).unsqueeze(2).expand(6, 256, 256)).int().unsqueeze(1)
...
if __name__ == "__main__":
    ...
    # -- path to save models
    pre_snapshot_path = "./model/BCP/ACDC_{}_{}_m_{}_lam_{}_labeled_exange_fea_out_final_have_ori_cps_loss_RE/pre_train".format(args.exp, args.labelnum, args.m, args.lam)
    self_snapshot_path = "./model/BCP/ACDC_{}_{}_m_{}_lam_{}_labeled_exange_fea_out_final_have_ori_cps_loss_RE/self_train".format(args.exp, args.labelnum,  args.m, args.lam)
    for snapshot_path in [pre_snapshot_path, self_snapshot_path]:
        if not os.path.exists(snapshot_path):
            os.makedirs(snapshot_path)
    shutil.copy('../code/train_ACDC_BCP_fix.py', self_snapshot_path)
